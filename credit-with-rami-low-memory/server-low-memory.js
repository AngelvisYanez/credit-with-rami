...
process.env.NODE_OPTIONS = '--max-old-space-size=1024 --max-semi-space-size=128';

const dev = process.env.NODE_ENV !== 'production';
const hostname = 'localhost';
const port = process.env.PORT || 3000;

// Crear la aplicación Next.js con configuración de memoria limitada
const app = next({ 
  dev, 
  hostname, 
  port,
...
app.prepare().then(() => {
  ...
  console.error('Error al preparar la aplicación:', ex);
  process.exit(1);
});
